...
describe.skip('PickerColumn boundary callbacks fire once', () => {
  ...
  it('fires boundary:hit event exactly once at min and once at max', async () => {
    ...
    await waitFor(() => {
      const minCalls = onGesture.mock.calls.filter(([event]) => event.type === 'boundary:hit' && event.boundary === 'min')
    ...
    await waitFor(() => {
      const maxCalls = onGesture.mock.calls.filter(([event]) => event.type === 'boundary:hit' && event.boundary === 'max')
