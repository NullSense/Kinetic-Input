...
type MockChildProps = {
  children?:
    | React.ReactNode
    | ((state: { selected: boolean; visuallySelected: boolean }) => React.ReactNode);
};
...
vi.mock('../../picker', () => {
  ...
  const Default = ({ value: _value, onChange, height, itemHeight, children }: MockPickerProps) => {
    ...
    return (
      <div data-testid="mock-picker" data-height={height} data-itemheight={itemHeight}>
        {children}
      </div>
    );
  };
  Default.Column = ({ children }: MockChildProps) => (
    <div data-testid="mock-column">{children}</div>
  );
  Default.Item = ({ children }: MockChildProps) => {
    const content =
      typeof children === 'function'
        ? children({ selected: false, visuallySelected: false })
        : children;
