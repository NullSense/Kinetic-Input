...
describe('useGestureCoordination', () => {
    ...
    it('opens picker on pointer down when closed', () => {
        ...
        const refs = {
            openedViaRef: { current: null } as React.MutableRefObject<'pointer' | 'wheel' | 'keyboard' | null>,
            currentGestureSource: { current: null } as React.MutableRefObject<'pointer' | 'wheel' | 'keyboard' | null>,
    ...
    it('handles wheel events by opening picker and scheduling idle timeout', () => {
        ...
        const { rerender } = renderHook(({ isOpen }: { isOpen: boolean }) =>
            useGestureCoordination({
                showPicker: isOpen,
                wrapperRef: { current: wrapper },
                handlePickerOpen,
    ...
    it('resets gesture state after momentum completes', () => {
        const wrapper = document.createElement('div');
        const refs = {
            openedViaRef: { current: 'pointer' } as React.MutableRefObject<'pointer' | 'wheel' | 'keyboard' | null>,
            currentGestureSource: { current: 'pointer' } as React.MutableRefObject<'pointer' | 'wheel' | 'keyboard' | null>,
        ...
        act(() => {
            result.current.onGesture({ type: 'boundary:hit', boundary: 'max', value: '9', timestamp: Date.now() });
            result.current.onGesture({ type: 'drag:end', hasMoved: true, velocity: 0, timestamp: Date.now() });
        });
